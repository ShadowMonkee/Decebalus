...
echo ""
echo -e "  ${YELLOW}${BOLD}Note — physical e-paper display (optional, Raspberry Pi).${NC}"
echo -e "  A mock display renders to the log by default. To drive a Waveshare e-paper HAT,"
echo -e "  enable SPI (\`sudo raspi-config\` → Interface Options → SPI) and build with:"
echo -e "    cargo build --release --features hardware"
echo ""
